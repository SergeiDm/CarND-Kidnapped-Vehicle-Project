  ...
  // Update particles locations
    ...
    if (yaw_rate==0) {
      ...
      double psi = particles[i].theta + yaw_rate * delta_t;
		  particles[i].x += (velocity/yaw_rate)*(sin(psi)- sin(particles[i].theta));
      particles[i].y += (velocity/yaw_rate)*(cos(particles[i].theta) - cos(psi));
		  particles[i].theta = psi;
    }
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[],
  ...
  // Update weights for every particle
	for (int i = 0; i < num_particles; ++i) {
      ...
      double numer = exp(-1*(dist_x*dist_x/(2*std_landmark[0]*std_landmark[0])+ dist_y*dist_y/(2*std_landmark[1]*std_landmark[1])));
